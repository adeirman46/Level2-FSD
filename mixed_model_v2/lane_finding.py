...
# def fit_poly(binary_warped, leftx, lefty, rightx, righty):
#     if len(rightx) == 0 or len(righty) == 0 or len(leftx) == 0 or len(lefty) == 0:
#         print("Error: Lane points array(s) are empty.")
#         # Handle the error gracefully, return default values or log the issue
...
def fit_poly(binary_warped, leftx, lefty, rightx, righty):
    ...
    try:
        ...
        # Check if any of the fitx arrays are None or invalid
        if left_fitx is None or right_fitx is None:
            print("Error: Polynomial fitting failed.")
            return None, None, None, None, ploty

    except TypeError as e:
        print(f"Error in polynomial fitting: {e}")
        # Provide default values if fitting fails
        left_fitx = np.ones_like(ploty)
        right_fitx = np.ones_like(ploty)
